<div class="row-fluid">
	<div class= "span13">
		<fieldset>
			
			<legend>Borrar Sección</legend>
			
			<div class="row-fluid">
				<div class="span5">
					
					<div class="row-fluid">
						<div class="span6">
							1.-Escoja una sección de la lista
						</div>
					</div>
					<div class="row-fluid">
						<div class="span10">
							<input type="text" id="info" class="span13" placeholder="Filtro búsqueda">
						</div>

					</div>
					<div class="row-fluid" style="border: 1px solid #cccccc; -webkit-border-radius: 4px; overflow-y:scroll; height:150px; margin-top: 2%; width:82%">


					</div>
					
					

				</div>
				<div class="span7">
					<div class="row-fluid">
						<div class="span8">
							2.-Información de la sección seleccionada
						</div>
					</div>
					<div class ="row-fluid">
						<div class="span10">
							<pre style=" padding: 2%">
Sección:									
Profesor:			
Módulo Actual:		
							</pre>
						</div>
					</div>
					<div class="row-fluid" >
						<div class="span8">
							3.-Alumnos de la sección
						</div>
					</div>
					<div class="row-fluid">
						<div class="span13">
							<table class="table table-bordered">
								<thead  bgcolor="#e6e6e6">
								    <tr>
								    	<th class="span2">Carrera</th>
								    	<th class="span2">RUT</th>
								    	<th class="span3">Paterno</th>
								    	<th class="span3">Materno</th>
								    	<th class="span9">Nombres</th>
								    </tr>
								</thead>
								    <!-- esta fila es solo de ejemplo-->
								<tbody>
								    <tr>
								    	<th>1853</th>
								    	<th>171233496</th>
								    	<th>Acevedo</th>
								    	<th>Baeza</th>
								    	<th>Marco Antonio</th>
								    </tr>
								</tbody>
								
							</table>
						</div>
					</div>
					<div class="row-fluid" style="margin-left:69%" >
						<div class="span2 ">
							<button class="btn" type="submit">Eliminar</button>
						</div>
						<div class="span3">
							<button class="btn" type="submit">Cancelar</button>
						</div>

					</div>
				</div>
			</div>
			
		</fieldset>
	</div>
</div>