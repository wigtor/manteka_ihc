...
class Salas extends MasterManteka {
	 ...
	 public function borrarSalas()
    {
		...
		$tipos_usuarios_permitidos[0] = TIPO_USR_COORDINADOR;
		$this->load->model('Model_sala');
		$datos_vista = array('sala' => $this->Model_sala->VerTodasLasSalas(),'mensaje_confirmacion'=>2, 'salaImplemento' => $this->Model_sala->VerTodosLosImplementosSala(),'mensaje_confirmacion'=>2);
    ...
    public function EliminarSalas($cod_sala)
    {
    	$datos_vista = 0;		
		$subMenuLateralAbierto = "borrarSalas"; 
		$muestraBarraProgreso = FALSE; //Indica si se muestra la barra que dice anterior - siguiente
