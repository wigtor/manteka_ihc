...
class model_planificacion extends CI_Model {
	...
	*/
	...
	/*	$sql = "SELECT  FROM WHERE  ";
		...
		$condiciones = '(sala_horario.COD_SECCION = seccion.COD_SECCION) AND (sala_horario.COD_SALA = sala.COD_SALA) AND (sala_horario.COD_HORARIO = horario.COD_HORARIO) AND (sesion.COD_SECCION = seccion.COD_SECCION) AND (sesion.COD_MODULO_TEM = modulo_tematico.COD_MODULO_TEM) AND (modulo_tematico.COD_EQUIPO = equipo_profesor.COD_EQUIPO) AND (equipo_profesor.COD_EQUIPO = profe_equi_lider.COD_EQUIPO) AND (profe_equi_lider.LIDER_PROFESOR = 1) AND (profe_equi_lider.RUT_USUARIO2 = profesor.RUT_USUARIO2) AND (modulo_tematico.COD_MODULO_TEM = equipo_profesor.COD_MODULO_TEM)';
		$desde = '`seccion`, `profesor`, `modulo_tematico`, `sesion`, `sala`, `horario`, `sala_horario`, `equipo_profesor`, `profe_equi_lider`';
	...
	SELECT seccion.COD_SECCION, profesor.NOMBRE1_PROFESOR, profesor.NOMBRE2_PROFESOR, profesor.APELLIDO1_PROFESOR, profesor.APELLIDO2_PROFESOR, modulo_tematico.NOMBRE_MODULO, sesion.DESCRIPCION_SESION, sala.NUM_SALA, horario.COD_DIA, horario.COD_MODULO FROM `seccion`, `profesor`, `modulo_tematico`, `sesion`, `sala`, `horario`, `sala_horario`, `equipo_profesor`, `profe_equi_lider` WHERE (sala_horario.COD_SECCION = seccion.COD_SECCION) AND (sala_horario.COD_SALA = sala.COD_SALA) AND (sala_horario.COD_HORARIO = horario.COD_HORARIO) AND (sesion.COD_SECCION = seccion.COD_SECCION) AND (sesion.COD_MODULO_TEM = modulo_tematico.COD_MODULO_TEM) AND (modulo_tematico.COD_EQUIPO = equipo_profesor.COD_EQUIPO) AND (equipo_profesor.COD_EQUIPO = profe_equi_lider.COD_EQUIPO) AND (profe_equi_lider.LIDER_PROFESOR = 1) AND (profe_equi_lider.RUT_USUARIO2 = profesor.RUT_USUARIO2) AND (modulo_tematico.COD_MODULO_TEM = equipo_profesor.COD_MODULO_TEM);	*/
